...
our $DESCRIPTOIN = "Cluster Service pool API가 변경한 내용이 시스템에 적용되는지 확인하는 테스트";
...
subtest 'Cluster Serivce Address API: deleted service pool application test' => sub
{
    ...
    subtest 'check permanent apply' => sub
    {
        ...
        ok(!$find_flag, "expanded service pool permanent apply check");
    };
    ...
    subtest 'check run-time apply' => sub
    {
        ...
        ok(!$find_flag, "expanded service pool run-time apply check");
    };
};
