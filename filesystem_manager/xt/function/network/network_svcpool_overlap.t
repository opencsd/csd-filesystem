...
our $DESCRIPTOIN = "Cluster Service pool API가 기존의 service pool과 겹치는 service pool의 생성을 방지하는지 확인하는 테스트";
...
subtest 'Cluster Serivce Address API: create a service pool' => sub
{
...
subtest 'Cluster Serivce Address API: create service pool overlapping created one' => sub
{
    my $t = Test::AnyStor::ClusterAddress->new(addr => $GMS_TEST_ADDR);
    ...
    ok($result, "overlapped service pool create check");
};

subtest 'Cluster Serivce Address API: reset service pool application test' => sub
{
    my $t = Test::AnyStor::ClusterAddress->new(addr => $GMS_TEST_ADDR);
