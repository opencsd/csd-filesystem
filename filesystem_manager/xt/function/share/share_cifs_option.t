...
our $DESCRIPTOIN = "Share API로 CIFS 옵션 설정이 잘 되는지 확인하는 테스트";
...
my $TEST_DIR = '/mnt/cifs_option_test';
...
subtest 'cifs_GuestAllow_test' => sub
{
    my $t = Test::AnyStor::Share->new(addr => $GMS_TEST_ADDR);
    ...
    # guest_allow_test
    system("ssh $GMS_CLIENT_ADDR mkdir -p $TEST_DIR &> /dev/null");

    subtest "guest_allow apply check" => sub
    {
        my %answer = (
            mountable => 0,
            readable  => 0,
        ...
        foreach my $key (keys(%answer))
        {
            ...
            my $correct_answer = $answer_sheet{guest_allow}{$key};
        ...
        ok($correct, "guest_allow check");
    };

    system("ssh $GMS_CLIENT_ADDR rm -rf $TEST_DIR &> /dev/null");

};

subtest 'cifs_HiddenShare_test' => sub
{
    my $t = Test::AnyStor::Share->new(addr => $GMS_TEST_ADDR);
    ...
    # hidden_share_test
    system("ssh $test_addr \"testparm -s --section-name test_share --parameter-name browseable &> /tmp/testparm\"");
    my $testparm = `ssh $test_addr "cat /tmp/testparm"`;
    system("ssh $test_addr \"rm -f /tmp/testparm\"");

    my @lines = split(/\n/, $testparm);
    my $browseable = $lines[@lines-1];
    chomp($browseable);

    ok($browseable eq 'No', 'hidden_share check');
};

goto INCOMPLETE;

subtest 'cifs_AuditOn_test' => sub
{
    my $t = Test::AnyStor::Share->new(addr => $GMS_TEST_ADDR);
