...
use Cluster::Initializer::NodeConfig;
...
my $INIT_ARGS = undef;
$INIT_ARGS = _get_config_conf($ENV{GMS_BUILD_CONFIG})
...
my $creation_steps = Cluster::Initializer::NodeConfig->new->get_steps_detail;
...
subtest 'Node Configuration rollback test' => sub
{
    ...
    for my $fault (@TESTS)
    {
        ...
        if ($err)
        {
            fail('Failed to trigger node configuration rollback');
            return;
        }

        $t->init_config($INIT_ARGS);
