package GMS::Controller::Cluster::Event;
...
sub event_list
{
    ...
    if (!defined($total))
    {
        $self->throw_error('Failed to get the number of event');
    }
    ...
    foreach my $event (@{$events})
    {
        ...
        $event->{Level} = grs_strlevel(delete($event->{Level}));
        $event->{Time}
            = strftime("%Y/%m/%d %T", localtime(delete($event->{Time})));
...
GMS::Controller::Cluster::Event - GMS API controller for event management features
